...
public class ProfileGUI {
    ...
    public static void open(
        ...
        if (targetPlayer != null) {
            ...
            jobsLore.add("§8• §7Joueur hors ligne");
        }

        if (jobsLore.isEmpty()) {
            jobsLore.add("§8• §7Aucun métier");
        }

        var resident = com.palmergames.bukkit.towny.TownyAPI
                .getInstance()
                .getResident(targetUUID);

        String townName = "Aucune ville";
        String nationName = "Aucune nation";
        boolean hasTown = false;
        boolean hasNation = false;

        if (resident != null && resident.hasTown() && resident.getTownOrNull() != null) {
            hasTown = true;
            townName = resident.getTownOrNull().getName();
        }

        if (resident != null && resident.hasNation() && resident.getNationOrNull() != null) {
            hasNation = true;
            nationName = resident.getNationOrNull().getName();
        }

        ItemStack identity = null;
        String source = "player";

        if (hasTown) {
            identity = MoodTownFlagAPI.getTownShieldItem(townName);
        ...
        if (identity == null && hasNation) {
            identity = MoodTownFlagAPI.getNationShieldItem(nationName);
        ...
        if (identity == null) {
            ...
            identity = new ItemStack(Material.PLAYER_HEAD);

            if (identity.getItemMeta() instanceof SkullMeta skullMeta) {
                skullMeta.setOwningPlayer(Bukkit.getOfflinePlayer(targetUUID));
                identity.setItemMeta(skullMeta);
            }
        }

        List<String> identityLore = new ArrayList<>();

        identityLore.add("§8• §7Joueur : §f" + shortText(name, 18));
        ...
        if (source.equalsIgnoreCase("town")) {
            ...
            identityLore.add("§8• §7Profil joueur affiché");
            identityLore.add("§8• §7Aucun blason défini");
        }

        identityLore.add("");
        identityLore.add("§8• §7Banque : §6" + SafeGUI.money(bank) + "€");
        identityLore.add("§8• §7Réputation : §a" + rep);
        identityLore.add("§8• §7Rang : " + rank);
        identityLore.add("");
        identityLore.add("§e➜ §fMétiers");
        ...
        for (String line : jobsLore) {
            ...
            identityLore.add("§8• §7" + shortText(cleanColor(line), 24));
            added++;
        }

        ItemMeta identityMeta = identity.getItemMeta();

        if (identityMeta != null) {
            identityMeta.setDisplayName("§6✦ §f" + shortText(name, 18) + " §6✦");
            identityMeta.setLore(identityLore);
            hide(identityMeta);
            identity.setItemMeta(identityMeta);
        }

        SafeGUI.safeSet(inv, 13, SafeGUI.glow(identity));
