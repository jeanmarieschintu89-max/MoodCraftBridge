...
import fr.moodcraft.bridge.listener.VaultBalanceTrackTask;
...
public class PriceHandler implements GUIHandler {
    ...
    private void sell(Player p, String id, Material mat, String display) {
        ...
        VaultBalanceTrackTask.suppress(p);
        VaultHook.getEconomy().depositPlayer(p, total);
        remove(p, mat, amount);
        MarketEngine.recordSell(id, amount);
        TransactionManager.marketSell(p.getUniqueId(), id, total, amount);
        ...
        if (total >= 50000) p.sendMessage("§e➜ §fVente majeure détectée.");
        footer(p);

        if (total >= 50000) {
            premiumClick(p, Sound.UI_TOAST_CHALLENGE_COMPLETE, 1f, Sound.BLOCK_BEACON_ACTIVATE, 1.2f);
        } else {
            premiumClick(p, Sound.ENTITY_EXPERIENCE_ORB_PICKUP, 1.15f, Sound.BLOCK_NOTE_BLOCK_CHIME, 1.35f);
        }

        p.sendTitle("§a+" + SafeGUI.money(total) + "€", "§fMarché §aMood§6Craft", 5, 35, 10);
    }
    ...
    private String cleanTrend(String trend) {
        if (trend == null || trend.isBlank()) return "§7Stable";
